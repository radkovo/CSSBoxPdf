...
public class LinearGradient {
	...
	/**
	 * This method is used for calculating coordinates of starting point and ending point of the linear gradient function.
	 *  @param angle angle of the gradient line.
	 ...
	 *  @param colorstops list of all colours in linear function.
	 *  @param dec decoder, which is used for decoding radial-gradient() parameters to value
	 *  @param isRepeating flag of repeating-linear-gradient() function
	 */
		...
		if (colorstops != null) {
			...
			for(int n = 0; n < colorstops.size(); n++) {
				...
				if ((colorstops.get(n).getLength() != null)) {
    				if (!colorstops.get(n).getLength().isPercentage()) 
    					isLength = true;
				}
			...
			else if(colorLen[colors.length - 2] != 1) { // if length 1 is not set in the end
				if (isRepeating){ // if repeating-linear-gradient() is set
			...
			while(n < trueIsLen.length) {
    			while(trueIsLen[n]) { // skipping colors with length
    				n++;
    			...
    			if (n >= trueIsLen.length)
    				break;
    			while((!trueIsLen[n]) && (n < trueIsLen.length)) { // calculating how many colors without lengths
    				n++;
    				m++;
    			}
    			if(m != 0) { // when is not end
	 ...
	 *  @param startx x-axis of the starting point of the gradient line
	 *  @param starty y-axis of the starting point of the gradient line 
	 *  @param endx x-axis of the ending point of the gradient line
	 *  @param endy y-axis of the ending point of the gradient line
	 */
	 ...
	 *  @return the COSArray, which is an important parameter for setting linear gradient.

...
public class RadialGradient {
	...
	/**
	 * This method is used for setting shape of gradient from the parameter of radial gradient function.
	 *  @param shapeIdent shape of the gradient.
	 */
	...
	/**
	 * This method is used for setting center point of gradient from the parameter of radial gradient function.
	 *  @param pos position of center point.
	 *  @param dec decoder, which is used for decoding radial-gradient() parameters to value
	...
	/**
	 * This method is used for setting radiuses of gradient from the parameter of radial gradient function.
	 *  @param size size parameter of radial function.
	 *  @param dec decoder, which is used for decoding radial-gradient() parameters to value
	...
	public void setRadiusFromSizeValue(TermLengthOrPercent[] size, CSSDecoder dec, int x, int y, int w, int h) {
		if (size != null) {
			if (size.length == 2) { // if ending shape is ellipse
	...
	/**
	 * This method is used for setting radiuses of gradient from the parameter of radial gradient function.
	 *  @param sizeIdent size parameter of radial function, which contains one of the keywords.
	 *  @param dec decoder, which is used for decoding radial-gradient() parameters to value
	 *  @param x x-axis of the element
	 *  @param y y-axis of the element
	 */
	 ...
	 *  @param colorstops list of all colours in radial function.
	 *  @param dec decoder, which is used for decoding radial-gradient() parameters to value
	 */
	public void createColorStopsLength(List<ColorStop> colorstops, CSSDecoder dec) {
		...
		else if(colorLen[colors.length - 2] != 1) { // if last length 1 is not set
		...
		while(n < trueIsLen.length) {
			while(trueIsLen[n]) { // colors with stop is skipped
				n++;
				if (n >= trueIsLen.length)
					break;
			}
			if (n >= trueIsLen.length)
				break;
			while((!trueIsLen[n]) && (n < trueIsLen.length)) { // calculating how many colors dont have declared stops
				n++;
				m++;
			}
			if(m != 0) { // if it is not the end
	...
	/**
	 * This method is used for creating ellipse shape from circular shape of radial function
	 ...
	 *  @param cx x-axis of the centre point of the element
	 *  @param cy y-axis of the centre point of the element
	 *  @param radius radius of the gradient
	 */
	public PDShadingType3 createRadialGrad(float cx, float cy, float radius) {
    	float[] components = new float[] { trueColors[0].getRed() / 255f, trueColors[0].getGreen() / 255f, trueColors[0].getBlue() / 255f };
	 ...
	 *  @return the function, which is an important parameter for setting radial gradient.
	 *  @param colors colours of radial gradient.
	 ...
	 *  @return the COSArray, which is an important parameter for setting radial gradient.
